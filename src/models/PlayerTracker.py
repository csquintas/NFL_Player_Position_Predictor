...
class SinusoidalPositionEncoder(nn.Module):
    ...
    def __init__(self, d_model: int, max_len: int = 4096):
        ...
        pos = torch.arange(0, max_len).unsqueeze(1).float() # frame positions
        div = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model)) # frequency scaling for even
